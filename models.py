...
class PretrainedResNetForBirdSnap10(torch.nn.Module):
    ...
    def setup_trainable_parameters(self, n_trainable_layers):
        ...
        self.trainable_layer_names = self.layer_names[-self.n_trainable_layers:] # DONE FIXME
        ...
        for name, param in self.model.named_parameters():
            ...
            if is_trainable:
                # DONE FIXME: modify param so it is trainable
                param.requires_grad = True 
                self.trainable_params[name] = param
                n_params += np.prod(param.shape)
            else:
                # DONE FIXME: modify param so it is NOT trainable
