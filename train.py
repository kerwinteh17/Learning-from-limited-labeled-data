    ...
    for epoch in progressbar:
        if epoch > 0:
            ...
            for bb, (x, y_B) in enumerate(tr_loader):
                ...
                # loss_xent = torch.tensor((0.4567,), device=device, requires_grad=True) # DONE FIXME 
                ...
                # loss_l2 = torch.tensor((0.0,),  device=device, requires_grad=True) # DONE FIXME 
                loss_l2 = torch.norm(model.trainable_params['output.weight'], p=2)
        ...
        for xva_B3HW, yva_B in va_loader:
            ...
            loss_xent_va = xent_loss_func(logits_BC, yva_B.to(device))
            
            batch_size = yva_B.size(0)
            va_xent += loss_xent_va.item() * batch_size  
        ...
        # Average the validation loss over the total number of samples
        va_xent /= n_valid
        ...
        # wait_enough = False # DONE FIXME. When should early stopping happen?
